...
const Link: InertiaLink = (props) => {
    ...
    const [local, rest] = splitProps(merged, [...Object.keys(defaults) as any, "href"])

    const as = () => local.as.toLowerCase()
    const method = () => local.method.toLowerCase() as Method
    const qs = () => mergeDataIntoQueryString(method(), local.href || "", local.data, local.queryStringArrayFormat)
    const href = () => qs()[0]
    const data = () => qs()[1]
    ...
    return (
        <Dynamic component={as()} {...rest} {...(as() === "a" ? { href: href() } : {})} onClick={visit}></Dynamic>
    )
}
